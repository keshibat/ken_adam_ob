require "application_system_test_case"

class SellersTest < ApplicationSystemTestCase
  setup do
    @seller = sellers(:one)
  end

  test "visiting the index" do
    visit sellers_url
    assert_selector "h1", text: "Sellers"
  end

  test "creating a Seller" do
    visit sellers_url
    click_on "New Seller"

    fill_in "Address", with: @seller.address
    fill_in "Company name", with: @seller.company_name
    fill_in "Cuisine", with: @seller.cuisine_id
    fill_in "Description", with: @seller.description
    fill_in "Google reviews url", with: @seller.google_reviews_url
    fill_in "Location", with: @seller.location_id
    fill_in "User", with: @seller.user_id
    fill_in "Website url", with: @seller.website_url
    click_on "Create Seller"

    assert_text "Seller was successfully created"
    click_on "Back"
  end

  test "updating a Seller" do
    visit sellers_url
    click_on "Edit", match: :first

    fill_in "Address", with: @seller.address
    fill_in "Company name", with: @seller.company_name
    fill_in "Cuisine", with: @seller.cuisine_id
    fill_in "Description", with: @seller.description
    fill_in "Google reviews url", with: @seller.google_reviews_url
    fill_in "Location", with: @seller.location_id
    fill_in "User", with: @seller.user_id
    fill_in "Website url", with: @seller.website_url
    click_on "Update Seller"

    assert_text "Seller was successfully updated"
    click_on "Back"
  end

  test "destroying a Seller" do
    visit sellers_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Seller was successfully destroyed"
  end
end
